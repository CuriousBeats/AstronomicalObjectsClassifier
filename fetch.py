...
url = SDSS_OBJ_SQL_SEARCH_BASE + "cmd=SELECT TOP 10 p.objid, s.specobjid, s.class, p.ra as ra, p.dec as dec, p.run AS r, p.camcol AS c, p.field as field FROM PhotoObj AS p JOIN SpecObj AS s ON s.bestobjid = p.objid&format=json"
testDataFetch(url)
